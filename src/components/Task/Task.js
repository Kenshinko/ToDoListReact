...
export default class TaskActive extends Component {
  ...
  render() {
    const { text, creationDate, isCompleted, isEditing, hasNoted, hasDeleted } = this.props
    ...
    return (
      <li className={isEditing ? 'editing' : isCompleted ? 'completed' : null}>
        ...
        {isEditing ? <input type="text" className="edit" placeholder="Editing task"></input> : null}
      </li>
    )
  }
}
